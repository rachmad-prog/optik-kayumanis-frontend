    ...
    <div className="mt-2">
      ...
      <div className="w-full aspect-[4/3] max-w-xs rounded-xl border border-sand overflow-hidden bg-cream mb-3">
        {value ? (
          isDirectVideoUrl(value) ? (
            ...
            <iframe src={toEmbedUrl(value)} className="w-full h-full" allow="autoplay; encrypted-media" allowFullScreen />
          ) : (
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <label className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full border border-cinnamon-300 text-cinnamon-600 hover:bg-cinnamon-50 cursor-pointer">
          <span>{uploading ? "Mengunggah..." : value ? "Ganti Gambar" : "+ Upload Gambar"}</span>
          <input
            type="file"
            accept="image/*"
            className="hidden"
...
export default function AdminContentPage() {
  ...
  async function uploadLayananMedia(file) {
    ...
    try {
      ...
      setStatusMap((m) => ({
        ...m,
        layanan: { ok: false, message: err.message || "Gagal mengunggah file." },
      }));
    } finally {
  ...
  async function uploadTentangMedia(file) {
    ...
    try {
      ...
      setStatusMap((m) => ({
        ...m,
        tentang: { ok: false, message: err.message || "Gagal mengunggah file." },
      }));
    } finally {
  ...
  return (
    <div>
      ...
      >
        ...
        <p className="text-sm font-semibold text-bark-700 mb-2 mt-4">Lokasi Peta</p>
        <Field
          label="Alamat atau link sematkan Google Maps"
          value={content.footer.mapEmbed || ""}
          onChange={(v) => update(["footer", "mapEmbed"], v)}
          hint='Isi alamat toko (mis. "Optik Kayumanis, Jl. Kayumanis No 12, Bogor"), atau tempel link dari Google Maps → Bagikan → Sematkan peta → salin src iframe-nya untuk kontrol titik lokasi yang presisi.'
        />

        <p className="text-sm font-semibold text-bark-700 mb-2 mt-4">Teks Hak Cipta (bawah footer)</p>
        <Field
