...
// Kunci unik perangkat USB (vendor+product+serial) supaya, kalau ada lebih
// dari satu device yang sudah pernah diizinkan browser, auto-reconnect tahu
// device mana yang terakhir dipakai sebagai printer.
function usbDeviceKey(device) {
  return `${device.vendorId}:${device.productId}:${device.serialNumber || ""}`;
}

function rememberUsbDevice(device) {
  try {
    localStorage.setItem("thermalPrinterUsbKey", usbDeviceKey(device));
  } catch {
    // localStorage tidak tersedia (mis. private mode) — auto-reconnect
    // di kunjungan berikutnya tidak akan jalan, tapi tidak fatal.
  }
}

// Buka device USB yang sudah dipilih/diizinkan (baik lewat requestDevice
// maupun getDevices), cari interface printer, dan klaim endpoint OUT-nya.
...
// Menampilkan dialog pemilihan device (wajib dipicu klik/tap oleh user —
// keterbatasan keamanan browser, tidak bisa dilewati).
export async function connectUsbPrinter() {
  ...
  const conn = await openUsbDevice(device);
  rememberUsbDevice(device);
  return conn;
}

// Coba sambung otomatis ke printer USB yang SUDAH PERNAH diizinkan
// sebelumnya (lewat connectUsbPrinter), TANPA memunculkan dialog pemilihan
// device — browser mengizinkan ini karena user sudah pernah memberi izin
// eksplisit untuk device tsb. Dipanggil otomatis saat halaman dibuka.
// Return null (diam-diam, tanpa error) kalau tidak ada device yang cocok,
// supaya tidak mengganggu user yang memang belum/tidak pakai printer USB.
export async function reconnectUsbPrinter() {
  ...
  let savedKey = null;
  try {
    savedKey = localStorage.getItem("thermalPrinterUsbKey");
  } catch {
    // ignore
  }
  const device = (savedKey && devices.find((d) => usbDeviceKey(d) === savedKey)) || devices[0];

  try {
    return await openUsbDevice(device);
  } catch {
    return null;
  }
}
