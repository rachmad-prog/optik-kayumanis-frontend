...
// Builds the "Hubungi Cabang" link so it stays in sync with whichever branch
// slide is currently active. If the configured CTA is a WhatsApp link
// (wa.me / api.whatsapp.com), the branch name is appended as a prefilled
// message ("...text=Halo, saya ingin bertanya tentang <Nama Cabang>") so the
// admin only has to manage one base WA number/link, while each branch slide
// still opens a chat that clearly references that specific branch. Non-WA
// links (e.g. Google Maps, custom URL) are left untouched.
function buildBranchCtaHref(baseHref, branchName) {
  ...
  params.set("text", `Halo, saya ingin bertanya tentang ${branchName}`);
      ...
      <div className="relative w-full max-w-5xl bg-cream-card rounded-2xl overflow-hidden">
        ...
        {(item.title || item.desc) && (
          <div className="p-5">
            ...
            {item.desc && (
              <p className="text-sm text-warmgray mt-2">{item.desc}</p>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
      <div className="max-w-2xl">
        ...
        {section?.subtitle && (
          <p className="text-warmgray mt-3">{section.subtitle}</p>
        )}
      </div>
      {children}
    </div>
    ...
    <section id="layanan" className="max-w-7xl mx-auto px-5 md:px-8 py-20">
      ...
      <button
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black/75 to-transparent text-cream">
          {current.title && (
            <h3 className="font-extrabold text-xl">{current.title}</h3>
          )}
          {current.desc && (
            <p className="text-sm text-cream/80 mt-1 max-w-2xl">
              {current.desc}
            </p>
          )}
          ...
          <button
            ...
            <div className="aspect-[16/9] bg-cream">
              {image ? (
                ...
                fallbackImage(current.title || section?.title)
              )}
            </div>
            <div className="p-5">
              <p className="text-base font-extrabold text-charcoal">
                {current.title || `Cabang ${active + 1}`}
              </p>
              {current.desc && (
                <p className="text-sm text-warmgray mt-2">{current.desc}</p>
              )}
            </div>
          </button>
          {items.length > 1 && (
  ...
  return (
    <section className="py-14 bg-cream-card border-y border-beige overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 md:px-8 mb-8">
        ...
        {sponsors?.subtitle && (
          <p className="text-warmgray mt-2">{sponsors.subtitle}</p>
        )}
      </div>
        ...
        <div className="sponsor-track flex gap-5 w-max mx-auto">
          {[...items, ...items, ...items].map((item, i) => (
            <div
              ...
              className="w-44 h-24 rounded-xl border border-beige bg-white flex items-center justify-center p-4">
              {item.image ? (
                <img
                  src={item.image}
                ...
                <span className="text-sm font-bold text-cinnamon">
                  {item.name || "Sponsor"}
                </span>
              )}
            </div>
  ...
  return (
    <section id="kontak" className="max-w-7xl mx-auto px-5 md:px-8 py-20">
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        {contact?.subtitle && (
          <p className="text-warmgray mt-3">{contact.subtitle}</p>
        )}
      </div>
      <div className="grid lg:grid-cols-2 gap-8 items-stretch">
        <form
          ...
          <label className="block mb-4">
            <span className="block text-sm font-semibold text-charcoal mb-1">
              Nama
            </span>
            <input
              required
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-6 py-3 rounded-full bg-cinnamon text-cream font-semibold hover:bg-cinnamon-700 transition">
            Kirim Pesan
          </button>
        </form>
        ...
        <div className="rounded-2xl overflow-hidden border border-beige bg-cream-card min-h-[360px]">
          {image ? (
            <img
              src={image}
              alt={contact?.title || "Kontak Optik Kayumanis"}
              className="w-full h-full object-cover"
            />
          ) : (
...
export default function HomeContentSections({ content }) {
  return (
    <>
      ...
      <CabangSlider
        section={content.cabang}
        ctaFromFooter={content.footer?.whatsappLink}
      />
