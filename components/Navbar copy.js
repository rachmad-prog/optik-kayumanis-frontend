    ...
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-charcoal text-cream/80 text-xs">
        <div className="max-w-7xl mx-auto px-5 md:px-8 py-2 flex justify-between items-center gap-3">
          ...
          <p className="mx-auto sm:mx-0 sm:mr-auto sm:ml-4 truncate">{topbar.topbarRight}</p>
          <button
            onClick={toggleLang}
          ...
          >
            <GlobeIcon />
            <span className="font-semibold tracking-wide">{lang === "en" ? "EN" : "ID"}</span>
          </button>
        </div>
      </div>
        ...
        <nav className="max-w-7xl mx-auto px-5 md:px-8 flex items-center justify-between h-20">
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="leading-tight">
              <p className="font-extrabold text-lg tracking-tight text-charcoal">Optik Kayumanis</p>
              <p className="text-[10px] uppercase tracking-widest text-warmgray">Eyewear &amp; Eyecare</p>
            </span>
          </Link>
          ...
          <ul className="hidden lg:flex items-center gap-8 text-sm font-medium text-charcoal">
            {navLinks.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="hover:text-cinnamon transition-colors">
                  {t(link.key)}
                </Link>
              </li>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <Link href="/account" className="text-sm font-semibold text-charcoal hover:text-cinnamon">
                  {t("hi")}, {user.name.split(" ")[0]}
                </Link>
                {user.role === "ADMIN" && (
                  <Link href="/admin" className="text-sm font-semibold text-cinnamon hover:text-cinnamon-700">
                    {t("admin")}
                  </Link>
                )}
                <button onClick={logout} className="text-sm font-semibold text-warmgray hover:text-cinnamon">
                  {t("logout")}
                </button>
              </div>
            ) : (
        ...
        {open && (
          <div className="lg:hidden border-t border-beige bg-cream px-5 pb-4">
            <ul className="flex flex-col gap-1 pt-2 text-sm font-medium text-charcoal">
              ...
              <li>
                <Link href="/cart" className="block py-2" onClick={() => setOpen(false)}>
              ...
              {user ? (
                <>
                  <li>
                    <Link href="/account" className="block py-2" onClick={() => setOpen(false)}>
                      {t("myAccount")}
                    </Link>
                  </li>
                  {user.role === "ADMIN" && (
                    <li>
                      <Link href="/admin" className="block py-2 text-cinnamon" onClick={() => setOpen(false)}>
                        {t("admin")}
                      </Link>
                    </li>
                  )}
                  <li>
                    <button onClick={logout} className="text-left py-2 text-warmgray w-full">
                      {t("logout")}
                    </button>
                  </li>
                </>
